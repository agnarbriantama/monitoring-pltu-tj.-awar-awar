...
function sfdialog(id, caption, size, fn) {
  ...
  **/
	$.post(
		"CIndex/getSfdialog",
		{ id: id, caption: caption, size: size },
		function (data) {
...
function sflookupevent(id, url, fn) {
	$("#" + id + " .modal-body input.q-search").on("keydown", function (e) {
		if (e.keyCode == 13) sflookupsearch(id, url, fn);
	});
	$("#" + id + " .modal-body span.btn-search").on("click", function (e) {
		sflookupsearch(id, url, fn);
	});
	$("#" + id + " .modal-body table tr.onclicktrlookup").on(
		"click",
		function () {
			if (fn) {
...
function exportExcel(id) {
	window.open(
		"data:application/vnd.ms-excel," + encodeURIComponent($("#" + id).html())
	);
}
